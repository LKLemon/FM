...
e.g., 24 million I/Q pairs (5 seconds at 2.4 Msamples/sec),
...
from fmSupportLib import fmDemodArctan, fmPlotPSD, my_lowpass, conv
...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		audio_coeff = signal.firwin(audio_taps, audio_Fc/(audio_Fs*audio_decim/2), window=('hann'))
	else:
		# to be updated by you for the takehome exercise
		# with your own code for impulse response generation
		audio_coeff = my_lowpass(audio_Fc,audio_Fs*audio_decim,audio_taps)
	...
	if il_vs_th == 0:
		...
		audio_filt = conv(audio_coeff,fm_demod)
