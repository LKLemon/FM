...
def my_fmDemodArctan(I, Q, prev_I_Q=[0.0, 0.0]):
	my_fm_demod = np.zeros(len(I))
		# iterate through each of the I and Q pairs
	for k in range(len(I)):
		...
		if (k == 0):
			if denom == 0:
				...
				my_fm_demod[k] = (I[k] * (Q[k] - prev_I_Q[1]) - Q[k] * (I[k] - prev_I_Q[0])) / denom
			# if(pow(I[k],2)+pow(Q[k],2)==0):
			# 	print(pow(I[k],2)+pow(Q[k],2))
			# 	print(k)
		else:
			if denom == 0:
				my_fm_demod[k] = my_fm_demod[k-1]
			else:
				my_fm_demod[k] = (I[k]*(Q[k]-Q[k-1])-Q[k]*(I[k]-I[k-1])) / denom
			# if(pow(I[k],2)+pow(Q[k],2)==0):
			# 	print(pow(I[k],2)+pow(Q[k],2))
			# 	print(k)
...
def my_lowpass(x_fc,x_fs,n):
	hI = []
	normcutoff = x_fc/(x_fs/2)
	for i in range(0,n):
		h_i = 0
		if i == (n-1)/2:
			h_i = normcutoff
		else:
			h_i= normcutoff * math.sin(math.pi*normcutoff*(i-(n-1)/2))/(math.pi*normcutoff*(i-(n-1)/2))
		hI.append(h_i * (math.sin(i*math.pi/n))**2)
...
def conv(h, x, state=[]):
	...
	taps_len = len(h)
	state_len = len(state)
	state = [0] * (taps_len-1) if state_len == 0 else state # set default state
	y = np.zeros(sig_len)

	for i in range(sig_len):
		for j in range(taps_len):
			if (i-j) < 0:
				y[i] += h[j] * state[i-j]
			else:
				y[i] += h[j] * x[i-j]
